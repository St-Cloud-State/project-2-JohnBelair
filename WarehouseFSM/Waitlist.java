...
public class Waitlist {
    ...
    public List<WaitlistItem> getWaitlistForClient(String clientId) {
     ...
     public List<WaitlistItem> getWaitlistByProductId(String productId) {
        List<WaitlistItem> result = new ArrayList<>();

        // Iterate over each client's waitlist in the map
        for (List<WaitlistItem> clientWaitlist : waitlistMap.values()) {
            for (WaitlistItem item : clientWaitlist) {
                // If the item matches the specified productId, add it to the result list
    ...
    public Map<String, List<WaitlistItem>> getwaitlist()
    {
        return waitlistMap;
    }
}
